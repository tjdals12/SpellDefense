...
namespace Repository.GoldShop {
    public class DataTableRepository : IRepository
    {
        ...
        void Awake() {
            ...
            this.goldShops = this.Load();
        }
        #endregion

        Entity[] Load() {
            #if UNITY_EDITOR
                TextAsset textAsset = Resources.Load<TextAsset>("DataTable/GoldShop");
                string text = textAsset.text;
            #else
                string filePath = Path.Combine(Application.persistentDataPath, "DataTable", "GoldShop.csv");
