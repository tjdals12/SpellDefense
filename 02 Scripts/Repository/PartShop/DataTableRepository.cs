...
namespace Repository.PartShop {
    public class DataTableRepository : IRepository
    {
        ...
        void Awake() {
            ...
            this.partShops = this.Load();
        }
        #endregion

        Dictionary<int, Entity[]> Load() {
            #if UNITY_EDITOR
                TextAsset textAsset = Resources.Load<TextAsset>("DataTable/PartShop");
                string text = textAsset.text;
            #else
                string filePath = Path.Combine(Application.persistentDataPath, "DataTable", "PartShop.csv");
